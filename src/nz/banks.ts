import { NzAccountError } from '../shared/errors.js';
import { err, ok, type Result } from '../shared/result.js';
import type {
  NzAccountParts,
  NzBankBranchRange,
  NzBankDefinition,
  NzChecksumRule
} from './types.js';

export const NZ_BANKS = [
  {
    id: '01',
    branches: [
      { from: 1, to: 999 },
      { from: 1100, to: 1199 },
      { from: 1800, to: 1899 }
    ]
  },
  {
    id: '02',
    branches: [
      { from: 1, to: 999 },
      { from: 1200, to: 1299 }
    ]
  },
  {
    id: '03',
    branches: [
      { from: 1, to: 999 },
      { from: 1300, to: 1399 },
      { from: 1500, to: 1599 },
      { from: 1700, to: 1799 },
      { from: 1900, to: 1999 }
    ]
  },
  { id: '04', branches: [{ from: 2020, to: 2024 }] },
  {
    id: '06',
    branches: [
      { from: 1, to: 999 },
      { from: 1400, to: 1499 }
    ]
  },
  { id: '08', branches: [{ from: 6500, to: 6599 }] },
  { id: '09', branches: [{ from: 0, to: 0 }] },
  { id: '10', branches: [{ from: 5165, to: 5169 }] },
  {
    id: '11',
    branches: [
      { from: 5000, to: 6499 },
      { from: 6600, to: 8999 }
    ]
  },
  {
    id: '12',
    branches: [
      { from: 3000, to: 3299 },
      { from: 3400, to: 3499 },
      { from: 3600, to: 3699 }
    ]
  },
  { id: '13', branches: [{ from: 4900, to: 4999 }] },
  { id: '14', branches: [{ from: 4700, to: 4799 }] },
  { id: '15', branches: [{ from: 3900, to: 3999 }] },
  { id: '16', branches: [{ from: 4400, to: 4499 }] },
  { id: '17', branches: [{ from: 3300, to: 3399 }] },
  { id: '18', branches: [{ from: 3500, to: 3599 }] },
  { id: '19', branches: [{ from: 4600, to: 4649 }] },
  { id: '20', branches: [{ from: 4100, to: 4199 }] },
  { id: '21', branches: [{ from: 4800, to: 4899 }] },
  { id: '22', branches: [{ from: 4000, to: 4049 }] },
  { id: '23', branches: [{ from: 3700, to: 3799 }] },
  { id: '24', branches: [{ from: 4300, to: 4349 }] },
  { id: '25', branches: [{ from: 2500, to: 2599 }] },
  { id: '26', branches: [{ from: 2600, to: 2699 }] },
  { id: '27', branches: [{ from: 3800, to: 3849 }] },
  { id: '28', branches: [{ from: 2100, to: 2149 }] },
  { id: '29', branches: [{ from: 2150, to: 2299 }] },
  { id: '30', branches: [{ from: 2900, to: 2949 }] },
  { id: '31', branches: [{ from: 2800, to: 2849 }] },
  { id: '33', branches: [{ from: 6700, to: 6799 }] },
  { id: '35', branches: [{ from: 2400, to: 2499 }] },
  { id: '38', branches: [{ from: 9000, to: 9499 }] },
  { id: '88', branches: [{ from: 8800, to: 8805 }] }
] as const satisfies readonly NzBankDefinition[];

const NZ_BANK_MAP = new Map<string, NzBankDefinition>(
  NZ_BANKS.map((bank) => [bank.id, bank])
);

function inRange(branch: string, range: NzBankBranchRange): boolean {
  const branchNumber = Number(branch);
  return branchNumber >= range.from && branchNumber <= range.to;
}

export function getNzBank(bankId: string): NzBankDefinition | undefined {
  return NZ_BANK_MAP.get(bankId.padStart(2, '0'));
}

export function isValidNzBankBranch(bankId: string, branch: string): boolean {
  const bank = getNzBank(bankId);

  if (!bank) {
    return false;
  }

  return bank.branches.some((range) => inRange(branch, range));
}

export function validateNzBankBranch(
  parts: Pick<NzAccountParts, 'bankId' | 'branch' | 'canonicalDigits'>
): Result<void, NzAccountError> {
  if (!isValidNzBankBranch(parts.bankId, parts.branch)) {
    return err(
      new NzAccountError(
        'NZ_ACCOUNT_BRANCH',
        `Account ${parts.canonicalDigits} failed validation: bank ${parts.bankId} does not allow branch ${parts.branch} in the bundled NZ bank table.`,
        {
          account: parts.canonicalDigits,
          bankId: parts.bankId,
          branch: parts.branch
        }
      )
    );
  }

  return ok(undefined);
}

export function selectNzChecksumRule(
  parts: Pick<NzAccountParts, 'bankId' | 'paddedBase'>
): NzChecksumRule {
  switch (parts.bankId) {
    case '08':
      return { algorithm: 'D', weightFactor: '000000076543210000', modulo: 11 };
    case '09':
      return { algorithm: 'E', weightFactor: '000000000054320001', modulo: 11 };
    case '25':
    case '33':
      return { algorithm: 'F', weightFactor: '000000017317310000', modulo: 10 };
    case '26':
    case '28':
    case '29':
      return { algorithm: 'G', weightFactor: '000000013713710371', modulo: 10 };
    case '31':
      return { algorithm: 'X', weightFactor: '000000000000000000', modulo: 1 };
    default:
      return Number(parts.paddedBase) < 990000
        ? { algorithm: 'A', weightFactor: '00637900A584210000', modulo: 11 }
        : { algorithm: 'B', weightFactor: '00000000A584210000', modulo: 11 };
  }
}
