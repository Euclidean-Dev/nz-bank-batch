...
function makeHeaderRecord(config: {
  ...
  return renderIb4bRecord([
    ...
    {
      value: config.userReference,
      spec: { name: 'userReference', maxLength: 12, truncate: true }
    }
  ]);
}
...
function createBnzFile(kind: BnzFileType, config: BnzFileConfig): BnzFile {
  const fromAccount = assertNzBankAccount(config.fromAccount, ['02'] as const);
  const processDate = normaliseProcessDate(config.processDate);
  const userReference = config.userReference ?? '';
  const transactionCode: BnzTransactionCode =
  ...
  const file: BnzFile = {
    kind,
    transactionCode,
    addTransaction(transaction: BnzTransaction) {
...
export function createDirectCreditFile(config: BnzFileConfig): BnzFile {
  return createBnzFile('direct-credit', config);
}

export function createDirectDebitFile(config: BnzFileConfig): BnzFile {
  return createBnzFile('direct-debit', config);
}

export function createFile(
  config: BnzFileConfig & { readonly type: BnzFileType }
): BnzFile {
  return createBnzFile(config.type, config);
}
