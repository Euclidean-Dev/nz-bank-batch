...
function makeHeaderRecord(config: {
  ...
  return renderIb4bRecord([
    ...
    { value: config.userReference, spec: { name: 'userReference', maxLength: 12, truncate: true } }
  ]);
}
...
function createBnzFile(kind: BnzFileType, config: BnzFileConfig): BnzFile {
  ...
  const transactionCode: BnzTransactionCode = kind === 'direct-credit' ? 'DC' : 'DD';
...
export function createFile(config: BnzFileConfig & { readonly type: BnzFileType }): BnzFile {
